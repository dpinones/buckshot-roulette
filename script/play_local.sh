...
BUYIN="1000000000000000000"  # 1 ETH
GAME_ID=0
MAX_TURNS=150
...
cast_send() {
  ...
  cast send --rpc-url "$RPC" --private-key "$pk" --gas-limit 500000 "$GAME_ADDR" "$@" 2>&1
}
...
echo ""
echo "[3/3] Creando partida 1v1..."

# Retry loop: la tx puede fallar por diferencias de block.prevrandao entre estimacion y ejecucion
GAME_CREATED=false
for ATTEMPT in 1 2 3 4 5; do
  CREATE_RESULT=$(cast send --rpc-url "$RPC" --private-key "$PK1" --gas-limit 600000 \
    "$GAME_ADDR" "createGame(address[],uint256)" "[$P1,$P2]" "$BUYIN" --value 2ether 2>&1)

  if tx_succeeded "$CREATE_RESULT"; then
...
echo "  Game ID: $GAME_ID | Prize Pool: 2 ETH"
echo "  P1: $P1"
echo "  P2: $P2"
